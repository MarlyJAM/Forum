...
if(isset($_GET['id']) AND !empty($_GET['id'])){
    ...
    $checkIfQuestionExists = $bdd->prepare('SELECT id_author FROM questions WHERE id = :id');
    ...
    if($checkIfQuestionExists->rowCount() > 0){
        ...
        if($questionsInfos['id_author'] == $_SESSION['id']){
            ...
            $deleteThisQuestion = $bdd->prepare('DELETE FROM questions WHERE id = :id');
            $deleteThisQuestion->execute(['id'=>$idOfTheQuestion]);
